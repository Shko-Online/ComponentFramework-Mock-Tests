...
describe("Metadata Tests", () => {
    ...
    it("can retrieve metadata", async () => {            
        ...
        const systemUserMetadata = await mockGenerator.context.utils.getEntityMetadata('systemuser');
    ...
    it("can retrieve subset of attributes", async ()=>{
        mockGenerator.metadata.initMetadata([JSON.parse(JSON.stringify(userMetadataJson))]);
        const systemUserMetadata = await mockGenerator.context.utils.getEntityMetadata('systemuser', ['fullname']);
